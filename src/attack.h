...
typedef struct s_attack {
	...
	struct dns_query    *queries_dns; /* array of dns_query struct */
	uint16_t		qr_count;
	uint16_t		qr_tries;
} Attack;
...
int add_query(const char *date, const char *hour, const char *client_ip, uint16_t port, const char *domain_name, const char *class_name, const char *type_name, char *status, const char *name_server, Attack *a);
int reassign_queries(Attack *dst, Attack *src);
#endif
