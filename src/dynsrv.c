...
void start_dynamic_server(struct in_addr ip_r, uint16_t port_r, struct in_addr ip_l, uint16_t port_l, uint16_t proto) {
	logmsg(LOG_DEBUG, 1, "Enter on start_dynamic_server.\n");

	pid_t			pid;
	int			listen_fd, mirror_sock_fd, proxy_sock_fd, connection_fd,established;
	bool handling_connection = false;  // Variable para controlar si se está manejando una conexión actualmente
	...
	if ((pid = myfork()) == 0) {
		...
		for (;;) {
			logmsg(LOG_DEBUG, 1, "Enter on endless loop in start_dynamic_server.\n");
			FD_ZERO(&rfds);
			FD_SET(sigpipe[0], &rfds);
			FD_SET(listen_fd, &rfds);

			logmsg(LOG_DEBUG, 1, "Value of sigpipe[0]: %d\n", sigpipe[0]);
			logmsg(LOG_DEBUG, 1, "Value of listen_fd: %d\n", listen_fd);
			logmsg(LOG_DEBUG, 1, "MAX(sigpipe[0], listen_fd): %d\n", MAX(sigpipe[0], listen_fd));
			c_timeout.tv_sec = conn_timeout;
			c_timeout.tv_usec = 0;

			logmsg(LOG_DEBUG, 1, "     Waiting for incoming connections.\n");

			switch (select(MAX(sigpipe[0], listen_fd) + 1, &rfds, NULL, NULL, &c_timeout)) {
			case -1:
				logmsg(LOG_DEBUG, 1, "Case -1\n");
				if (errno == EINTR) {
			...
			case 0:
				/* timeout */
				logmsg(LOG_DEBUG, 1, "Case 0:     Timeout.\n");
				logmsg(LOG_NOISY, 1,
				...
				exit(EXIT_SUCCESS);
			default:
				logmsg(LOG_DEBUG, 1, "     Enter on default.\n");
				if (FD_ISSET(sigpipe[0], &rfds) && (check_sigpipe() == -1)) exit(EXIT_FAILURE);
				if (FD_ISSET(listen_fd, &rfds)) {
		/*			if (handling_connection) {
				            // Ya se está manejando una conexión, omitir la aceptación de la nueva conexión
			        	    logmsg(LOG_DEBUG, 1, "Skipping new connection because a connection is already being handled.\n");
				            continue;
        				}*/

					logmsg(LOG_DEBUG, 1, "     FD_ISSET correct.\n");
					/* accept connection depending on protocol */
					bzero(&cliaddr, sizeof(cliaddr));
					logmsg(LOG_DEBUG, 1, "bzero OK.\n");
					cliaddr_len = sizeof(cliaddr);
					established = 0;

					switch ((uint16_t) proto) {
					case TCP:
						logmsg(LOG_DEBUG, 1, "     Accept connection TCP.\n");
						/* accept tcp connection request */
						logmsg(LOG_DEBUG, 1, "Calling accept().\n");
						...
						logmsg(LOG_DEBUG, 1, "accept() OK.\n");

						// set keepalive option on connected socket
						int sockopt = 1;
						logmsg(LOG_DEBUG, 1, "Calling setsokopt().\n");
						...
						established = 1;
						logmsg(LOG_DEBUG, 1, "setsokopt() OK.\n");
						break;
					case UDP:
						logmsg(LOG_DEBUG, 1, "     Accept connection UDP.\n");
						...
						logmsg(LOG_DEBUG, 1, "Calling connect().\n");
						if (connect
						...
						logmsg(LOG_DEBUG, 1, "connect().\n");

						/* update remote endpoint information for attack structure */
						logmsg(LOG_DEBUG, 1, "calling getpeername().\n");
						...
						logmsg(LOG_DEBUG, 1, "getpeername() OK.\n");
						established = 1;
						break;
					default:
		...
		//			handling_connection = true;  // Se está manejando una conexión
					...
					logmsg(LOG_DEBUG, 1, "getsockname() OK.\n");
					...
					logmsg(LOG_DEBUG, 1, "Initialize new attack record.\n");
					...
					close(connection_fd);
					free(attack);
		//			handling_connection = false;  // Se ha terminado de manejar la conexión
...
void handle_connection(int connection_fd, struct sockaddr_in cliaddr, struct sockaddr_in srvaddr, uint16_t port_l, uint16_t proto, int mirror_sock_fd, int proxy_sock_fd, Attack *attack) {
    char *ip_l_str, *ip_r_str;

    ip_l_str = strdup(inet_ntoa(srvaddr.sin_addr));
    ip_r_str = strdup(inet_ntoa(cliaddr.sin_addr));

    logmsg(LOG_NOTICE, 1, "   Handling %s connection request from %s:%d to %s:%d.\n",
           PROTO(proto), ip_r_str, ntohs(cliaddr.sin_port), ip_l_str, ntohs(srvaddr.sin_port));

    // Resto del código de gestión de la conexión...
...
int handle_connection_normal(int connection_fd, uint16_t port, uint16_t proto, u_char timeout, Attack * attack) {
	...
	logmsg(LOG_INFO, 1, "Handling normal connection.\n");
	...
	for (;;) {
		...
		default:
			...
			if (FD_ISSET(connection_fd, &rfds)) {
				...
				logmsg(LOG_INFO, 1, "Data available on socket\n");
