...
const char *https_program;
const char *https_options;
...
conf_node *plugin_process_confopts(conf_node *tree, conf_node *node, void *opt_data) {
	...
	while (node->val) {
		...
		if OPT_IS("http_program") {
			...
			http_options = value;
		} else if("https_program"){
			https_program = value;
		}
		else if("https_options"){
			https_options = value;
		}
		else {
...
int cmd_parse_for_http_url(Attack *attack) {
	...
	for (i=0; i<attack->a_conn.payload.size; i++) {
		...
		//for https 
		else if((attack->a_conn.payload.size -i >= 8) && (memcmp(string_for_processing + i, "https://", 8) == 0)){
			start = string_for_processing+i;
			...
			logmsg(LOG_DEBUG, 1, "HTTPS download - URL found: '%s'\n", start);

			// increase number of download tries
			attack->dl_tries++;

			/* assemble wget download command and execute it */
			if (asprintf(&cmd, "%s %s %s -q", https_program, https_options, start) == -1) {
				logmsg(LOG_ERR, 1, "HTTP download error - Unable to allocate memory: %s.\n", strerror(errno));
				free(string_for_processing);
				free(cmd);
				return(-1);
			}
			logmsg(LOG_DEBUG, 1, "HTTPS download - Calling '%s'.\n", cmd);
			if ((f = popen(cmd, "r")) == NULL) {
				logmsg(LOG_ERR, 1, "HTTPS download error - Cannot call download command: %m.\n");
				free(string_for_processing);
				free(cmd);
				return(0);
			}
			...
			do {
				if ((binary_stream = realloc(binary_stream, total_bytes + BUFSIZ)) == NULL) {
					logmsg(LOG_ERR, 1, "HTTPS download error - Unable to allocate memory: %s.\n", strerror(errno));
					pclose(f);
					free(string_for_processing);
					free(cmd);
					free(binary_stream);
			...
			// add download to attack record
			if (total_bytes) {
				logmsg(LOG_DEBUG, 1, "HTTPS download - Adding download to attack record.\n");

				add_download("https", TCP, 0, 0, NULL, NULL, strrchr(start, '/')+1, start, binary_stream, total_bytes, attack);

				logmsg(LOG_INFO, 1, "HTTPS download - %s successfully downloaded and attached to attack record.\n", start);
			} else logmsg(LOG_INFO, 1, "HTTPS download - No data received.\n");
...
int is_https(char *request){
	if(strcmp(request, "https://")){
		return 1; // is HTTPS
	}
	else if(strcmp(request, "http://")){
		return 0; // is HTTPS
	}
	else{
