...
void server_wrapper(u_char *args, const struct pcap_pkthdr *pheader, const u_char * packet) {
        ...
        logmsg(LOG_DEBUG, 1, "Enter on server_wrapper.\n");
	...
	// Obtener la dirección IP local del sistema
        if ((status = getaddrinfo("localhost", NULL, &hints, &res)) != 0) {
		fprintf(stderr, "Error al obtener la dirección IP local: %s\n", gai_strerror(status));
		exit(EXIT_FAILURE);
	}

	// Recorrer la lista de direcciones y obtener la primera dirección IPv4
	for (p = res; p != NULL; p = p->ai_next) {
        ...
        // Liberar la memoria de la lista de direcciones
	freeaddrinfo(res);

        ip = (struct ip_header *) (packet + pcap_offset);
        if (ip->ip_p == TCP) {
                logmsg(LOG_DEBUG, 1, "Protocol = TCP.\n");
                ...
                logmsg(LOG_DEBUG, 1, "Protocol = ICMP.\n");
                ...
                logmsg(LOG_DEBUG, 1, "Protocol = UDP.\n");
                udp = (struct udp_header *) ((u_char *) ip + (4 * ip->ip_hlen));
                sport = ntohs(udp->uh_sport);
                dport = ntohs(udp->uh_dport);
                port_mode = port_flags_udp[dport] ? port_flags_udp[dport]->mode : 0;
        ...
        logmsg(LOG_DEBUG, 1, "Calling start_dynamic_server.\n");
...
int start_pcap_mon(void) {
        ...
        if (!dev) {
                ...
                if (pcap_findalldevs(&alldevs, errbuf) == -1) {
                        logmsg(LOG_ERR, 1, "Error - Could not retrieve device list: %s\n", errbuf);
                        exit(EXIT_FAILURE);
                }

                if (alldevs == NULL) {
                        logmsg(LOG_ERR, 1, "Error - No devices found.\n");
                        exit(EXIT_FAILURE);
                }
        ...
        if ((packet_sniffer = pcap_open_live(dev->name, BUFSIZ, promisc_mode, 10, errbuf)) == NULL) {
        ...
        for (;;) {
                logmsg(LOG_DEBUG, 1, "Enter on endless loop in start_pcap_mon().\n");
                FD_ZERO(&rfds);
                FD_SET(sigpipe[0], &rfds);
                FD_SET(pcap_fd, &rfds);

                logmsg(LOG_DEBUG, 1, "Value of sigpipe[0]: %d\n", sigpipe[0]);
                logmsg(LOG_DEBUG, 1, "Value of pcap_fd: %d\n", pcap_fd);
                logmsg(LOG_DEBUG, 1, "MAX(sigpipe[0], pcap_fd): %d\n", MAX(sigpipe[0], pcap_fd));

                switch (select(MAX(pcap_fd, sigpipe[0]) + 1, &rfds, NULL, NULL, &mainloop_timeout)) {
                case -1:
                        logmsg(LOG_DEBUG, 1, "Case -1. Error en select().\n");
                        if (errno == EINTR) {
                ...
                case 0:
                         ...
                         */
                        logmsg(LOG_DEBUG, 1, "Calling pcap_dispatch().\n");
                        ...
                        break;
                default:
                        logmsg(LOG_DEBUG, 1, "Default. select() OK\n");
