...
#define MAX_PORTS 65536 // Número máximo de puertos posibles

int active_connections[MAX_PORTS] = {0}; // Inicializa el array con ceros
...
int get_boundsock(struct sockaddr_in *server_addr, uint16_t port, int type) {
	...
	if (setsockopt(fd, SOL_SOCKET, SO_REUSEADDR, &reuse, sizeof(reuse)) < 0) {
		logmsg(LOG_ERR, 1, "Error - Could not set SO_REUSEADDR for server socket: %m.\n");
		exit(EXIT_FAILURE);
	}

	// Si el puerto ya está ocupado, cierra el socket y devuelve -1
    	if (errno == EADDRINUSE) {
        	close(fd);
        	return(-1); // Indica que el puerto está ocupado
	}
	...
	if (rv != 0) {
		...
		default:
			...
			if (FD_ISSET(sock_fd, &rfds) || FD_ISSET(sock_fd, &wfds)) {
				...
				if (error) {
					errno = error;
					// Si el error es EADDRINUSE, cierra el socket y devuelve -1
                			if (errno == EADDRINUSE) {
		                        	close(sock_fd);
                        			return(-1); // Indica que el puerto está ocupado
                    			}	
